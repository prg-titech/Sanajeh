...
class Requestor:
  ...
  def Requestor(self):
    pass
  def DoSuccess(self, other: Acceptor):
    pass
  def DoFail(self, other: Acceptor):
    pass

class Acceptor:
  def __init__(self):
    self.request_list: list[Requestor] = ([None] * 5)
    self.selected: Requestor = None
    self.atomic_request: int = 0
    self.max_request: int = 0

  def Acceptor(self):
    self.atomic_request = 0
    self.max_request = 5

  def RequestTicket(self, target: Requestor):
    ticket: int = DeviceAllocator.atomicAdd( self.atomic_request, 1 )
    if ticket<=self.max_request:
      self.request_list[ticket] = target

  def DecideRequestList(self):
    self.PredicateList()
    if self.selected != None:
      self.selected.DoSuccess( self )
    for x in range(self.max_request):
      candidate: Requestor = self.request_list[x]
      ...
      for dx in range(-self.vision_,self.vision_+1):
        for dy in range(-self.vision_,self.vision_+1):
          ...
          if nx >= 0 and nx < kSize and ny >= 0 and ny < kSize:
            n_id: int = nx + ny*kSize
            n_cell: Cell = cells[n_id]
            n_female: Female = DeviceAllocator.cast( Female, n_cell.agent() )
  ...
  def mate(self):
    if self.proposal_accepted_ and self.permission_:
      ...
      if self.random_float() <= 0.5:
        ...
        child = DeviceAllocator.new(Female, self.cell_request_ref, c_vision, 0, c_max_age, c_endowment, c_metabolism, self.female_request_ref.max_children())
      ...
      for dx in range(-kMaxVision,kMaxVision+1):
        for dy in range(-kMaxVision,kMaxVision+1):
          ...
          if nx >= 0 and nx < kSize and ny >= 0 and ny < kSize:
            n_id: int = nx + ny*kSize
            n_cell: Cell = cells[n_id]
            n_male: Male = DeviceAllocator.cast( Male, n_cell.agent() )
