...
class Body():
    ...
    def Body(self, idx: int):
        ...
        self.pos = Vector(((2.0 * random.uniform()) - 1.0), ((2.0 * random.uniform()) - 1.0))
        self.vel = Vector(((random.uniform() - 0.5) / 1000), ((random.uniform() - 0.5) / 1000))
    ...
    def body_update(self):
        __auto_v0: Vector = self.force.multiply(kTimeInterval)
        __auto_v1: Vector = __auto_v0.divide(self.mass)
        self.vel.add(__auto_v1)
        __auto_v2: Vector = self.vel.multiply(kTimeInterval)
    ...
    def update_merge(self):
        ...
        if (m is not None):
            if (m.merge_target_ref is not None):
                ...
                __auto_v0: Vector = self.vel.multiply(self.mass)
                __auto_v1: Vector = m.vel.multiply(m.mass)
                __auto_v2: Vector = __auto_v0.plus(__auto_v1)
                new_vel: Vector = __auto_v2.divide(new_mass)
                m.mass = new_mass
                m.vel = new_vel
                __auto_v3: Vector = self.pos.add(m.pos)
                m.pos = __auto_v3.divide(2)
