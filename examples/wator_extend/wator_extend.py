...
class BreedBehavior():
  ...
  def breed_prepare(self):
    self.egg_timer_ += 1

  def newling(self) -> Agent:
    ret: Agent = DeviceAllocator.virtual(BreedBehavior.newling)
    return ret

  def breed_update(self, position: Cell):
      if self.egg_timer_ > kSpawnThreshold:
        newling: Agent = self.newling()
        assert newling != None
...
class Fish(Agent,BreedBehavior):
  ...
  def update(self):
    ...
    if old_position != self.new_position_ref:
      ...
      self.breed_update(old_position)
