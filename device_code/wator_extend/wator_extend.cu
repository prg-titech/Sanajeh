...
__device__ void BreedBehavior::BreedPrepare() {
	this->egg_timer_ += 1;
}

__device__ Agent* BreedBehavior::BreedNewling() {
	Agent* ret = this->cast<Fish>() != nullptr ? this->cast<Fish>()->BreedNewling() :
this->cast<Shark>() != nullptr ? this->cast<Shark>()->BreedNewling() :
nullptr;
	return ret;
}

__device__ void BreedBehavior::BreedUpdate(Cell* position) {
	if (this->egg_timer_ > kSpawnThreshold) {
		Agent* newling = this->BreedNewling();
...
__device__ void Fish::update() {
	...
	if (old_position != this->new_position_ref) {
		...
		this->BreedUpdate(old_position);
	}
}

__device__ Agent* Fish::BreedNewling() {
...
__device__ Agent* Shark::BreedNewling() {
