...
__device__ void Cell::setup() {
	int x = this->id % kSizeX;
	int y = this->id // kSizeX;
...
__device__ void Cell::decide() {
	if (this->neighbor_request_[4]) {
		...
		if (num_candidates > 0) {
			...
			this->neighbors_[candidates[selected_index]]->agent()->set_new_position(this);
		}
	}
}
...
__device__ void Cell::request_random_fish_neighbor() {
	if (!this->request_random_neighbor_has_fish(this->agent_ref->random_state_)) {
		if (!this->request_random_neighbor_is_free(this->agent_ref->random_state_)) {
...
__device__ bool Cell::request_random_neighbor_has_fish(curandState& random_state) {
	int candidates[4];
	int num_candidates = 0;
	int i = 0;
	while (i < 4) {
...
__device__ bool Cell::request_random_neighbor_is_free(curandState& random_state) {
	int candidates[4];
	int num_candidates = 0;
	int i = 0;
	while (i < 4) {
