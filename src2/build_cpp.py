...
class InitializerList:
    ...
    def buildCpp(self, ctx):
        ...
        return "this->{}::{}__init({});".format(self.super_class,self.super_class, ", ".join([x.buildCpp(ctx) for x in self.args]))
        ...
        with BuildContext(ctx, self) as new_ctx:
            if ctx.in_class():
                ...
                if self.name == ctx.stack[-1].name:
                    ...
                    # EXTENSION: init list must be none, because we call default constructor instead
                    ret = "\n".join([
                        "\n{}__device__ {}::{}({}) {{".format(
                            ctx.indent(),
                            ctx.stack[-1].name,
                    ...
                    # Duplicate constructor as init method
                    arg_count = len(self.args.args)-1
                    if arg_count>0:
                        ret += "\n".join([
...
class ClassDef(CodeStatement):
    ...
    def HasDefaultConstructor(self):
        body = self.stmt
        for func in body:
            if type(func) is FunctionDef:
                if func.name==self.name:
                    # This class' constructor, check its argument list
                    return len(func.args.args)<=1

    def CheckParent( self, cls ):
        for base in self.bases:
            # find ClassDef of parent for this base
            ref = next( (x for x in cls if x.name==base.id ) , None )
            if ref is not None:
                self.parents.append(ref)
    ...
    def buildHpp(self, ctx):
        with BuildContext(ctx, self) as new_ctx:
            ...
            #         do_field_types.append(type_converter.do_all_convert(parent.fields[field]))
...
class Call(CodeExpression):
    ...
    def buildCpp(self, ctx):
        if hasattr(self.func, "value") and hasattr(self.func.value, "id") and self.func.value.id == "DeviceAllocator":
            ...
            elif self.func.attr == "virtual":
                # need data for this variables
                func_name = self.args[0].attr
                children = ["Fish","Shark"]

                ret = ""
                for child in children:
                    ret += "this->cast<{}>() != nullptr ? this->cast<{}>()->{}() :\n".format(child,child,func_name)
                ret += "nullptr"

                return ret
            elif self.func.attr == "atomicAdd":
                return "atomicAdd(&{},{})".format( self.args[0].buildCpp(ctx), self.args[1].buildCpp(ctx) )
            elif self.func.attr == "cast":
                return "{}->cast<{}>()".format(self.args[1].buildCpp(ctx),self.args[0].buildCpp(ctx))
            else:
                # todo: unprovided sanajeh API
                print("unknown",self.func.attr)
                assert False
            """
...
class Cast(Call):
    def buildCpp(self,ctx):
        args = ", ".join([x.buildCpp(ctx) for x in self.args])
        return "{}->cast<{}>()".format(args,self.func)
