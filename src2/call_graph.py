...
class CallGraph(CallGraphNode):
    ...
    def __init__(self, node_name):
        ...
        random_class: ClassNode = ClassNode("random")
        ...
        da_class: ClassNode = ClassNode("DeviceAllocator")
    ...
    def MarkDeviceDataByClassName(self, class_names):
        for cln in class_names:
            ...
            for cls in self.declared_classes:
                if cln in cls.super_class and cls.name not in class_names \
            ...
            clsnode = self.GetClassNode(cln)
            parents = clsnode.super_class
            if len(parents)>0:
                self.MarkDeviceDataByClassName(clsnode.super_class)
