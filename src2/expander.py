...
class RuntimeExpander:
  ...
  def has_random_state(self, cls, py_code):
    # add inheritance
    bases = [x.__name__ for x in cls.__bases__ if x.__name__!='object']
    bases.append(cls.__name__)

    tree = ast.parse(py_code)
    for body in tree.body:
      if type(body) == ast.ClassDef and body.name in bases:
  ...
  def build_function(self, cls, py_code):
    ...
    # add inheritance
    original_field_dict = self.get_dict(self.get_init(cls))
    field_dict = {}
    # for base in cls.__bases__:
    #   if base.__name__ == 'object':
    #     continue
    #   parent_dict = self.get_dict(self.get_init(base))
    #   field_dict.update(parent_dict)
    field_dict.update(original_field_dict)
