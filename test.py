...
print("compile time(py2cpp): %dµs" % ((compile_time - start_time) * 100000))
print("compile time(nvcc): %dµs" % ((build_time - compile_time) * 100000))
print("initialize time: %dµs" % ((initialize_time - build_time) * 100000))
print("parallel new time(%-5d objects): %.dµs" % (obn, ((parallel_new_time - initialize_time) * 100000)))
print("average computation time: %dµs" % ((end_time - parallel_new_time) * 100000 / itr))
print("overall computation time(%-4d iterations): %dµs" % (itr, ((end_time - parallel_new_time) * 100000)))
# print("do_all time: %dµs" % ((end_time2 - end_time) * 100000))
print("overall time: %dµs" % ((end_time2 - start_time) * 100000))
